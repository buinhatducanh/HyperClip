...
export function registerAuthHandlers(ipcMain: IpcMain): void {
  ...
  // ── OAuth Credentials ────────────────────────────────────────────────────
  ipcMain.handle(IPC_CHANNELS.AUTH_OAUTH_SET_CREDS, (_, clientId: string, clientSecret: string) => {
      ...
      if (fs2.existsSync(configFile)) {
        ...
        if (typeof config === 'object') {
          ...
          for (const pid of ['proj-01', 'proj-02', 'proj-03', 'proj-04']) {
            if (config[pid]?.clientId) result[pid] = { clientId: config[pid].clientId }
          }
          if (config.client_id) result['_default'] = { clientId: config.client_id }
          return result
        }
      }
    } catch {}
