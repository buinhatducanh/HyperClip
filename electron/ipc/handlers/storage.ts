...
type NotificationFn = (type: 'success' | 'error' | 'warning' | 'info', message: string, workspaceId?: string) => void

export function registerStorageHandlers(
  ipcMain: IpcMain,
  sendNotification: NotificationFn,
): void {
  ipcMain.handle(IPC_CHANNELS.STORAGE_GET_SIZE, async (): Promise<{
    downloads: number; blur: number; total: number
    downloadPath: string; outputPath: string; freeBytes: number
  }> => {
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.STORAGE_CLEAR_DOWNLOADS, async (): Promise<{ success: boolean; freedMB: number }> => {
    try {
      ...
      sendNotification('error', `Clear failed: ${(err as Error).message}`)
