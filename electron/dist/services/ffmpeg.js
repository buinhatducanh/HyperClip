...
function buildFilterComplex(opts) {
    const { headerOl, titleOl, canvasW, canvasH, headerH, titleH, videoH, videoTop, videoW, speedFilter, backgroundType = 'blur', titleOverlayPath, isShort = true, useCuda = true, fpsTarget = 30, } = opts;
    ...
    if (!isShort) {
        ...
        if (titleOl?.content) {
            ...
            if (titleOverlayPath) {
                ...
                const drawtext = `drawtext=text='${escapedText}':fontsize=${fontSize}:fontcolor=white:borderw=2:bordercolor=${borderColor}:x=(w-text_w)/2:y=(h-text_h)/2:fontfile=${FONT_FILE}`;
    ...
    if (titleOl?.content) {
        if (titleOverlayPath) {
            ...
            const escapedText = titleOl.content.replace(/'/g, "\\'").replace(/:/g, "\\:").replace(/\[/g, "\\[").replace(/\]/g, "\\]");
            const borderColor = titleOl.borderColor ?? '#00B4FF';
            const drawtext = `drawtext=text='${escapedText}':fontsize=${fontSize}:fontcolor=white:borderw=2:bordercolor=${borderColor}:x=(w-text_w)/2:y=${titleCenterY}-(text_h/2):fontfile=${FONT_FILE}`;
            ...
            if (hdChain) {
                ...
                if (hdIdx !== -1) {
                    sections.splice(hdIdx, 1, `[vz][texted]${overlay}=0:${videoTop}[texted_vz];` +
...
function getNvencParams(codec, isChunked, gpuTier = 'software', canvasW = 0, canvasH = 0) {
...
export async function renderTextOverlay(text, canvasW, canvasH, headerH, titleH, videoTop, borderColor, bgColor, fontSize, outputPath) {
    ...
    `color=${borderColor}:alpha=${alpha}:s=${boxW}x${boxH}:d=1:r=1,` +
        ...
        `drawtext=text='${escapedText}':fontsize=${fs2}:fontcolor=white:borderw=2:bordercolor=${borderColor}:x=(w-text_w)/2:y=(h-text_h)/2:fontfile=${FONT_FILE}[texted];` +
...
export async function renderVideo(metadata, outputDir, onProgress, gpuTier = 'software') {
    const { workspace_id, source_video, export_resolution, video_speed, fps_target, overlays, trim, codec = 'h264', backgroundType = 'blur', backgroundColor = '#000000', backgroundImage, blur_background, isShort = true, vidHeightPct = 50, audioCodec = 'aac', audioBitrate = '192k', } = metadata;
    ...
    const decCodec = getBestHwDecCodec(codec);
    ...
    if (!isShort) {
        ...
        if (titleOl?.content) {
            if (titleOverlayPath) {
                ...
                const escapedText = (titleOl.content || '').replace(/'/g, "\\'").replace(/:/g, "\\:").replace(/\[/g, "\\[").replace(/\]/g, "\\]");
                const borderColor = titleOl.borderColor ?? '#00B4FF';
                sections.push('[vz]drawtext=text=\'' + escapedText + '\':fontsize=' + fontSize + ':fontcolor=white:borderw=2:bordercolor=' + borderColor + ':x=(w-text_w)/2:y=(h-text_h)/2:fontfile=' + FONT_FILE + '[td]');
            }
        }
...
export async function renderChunked(metadata, outputDir, config = {}, onProgress) {
    ...
    for (let batchStart = 0; batchStart < numChunks; batchStart += workers) {
        ...
        const batchResults = await Promise.all(batch.map(async (idx) => {
            ...
            const result = await encodeChunk(workspace_id, source_video, blur_background || '', startSec, durationSec, chunkFile, codec, canvasW, canvasH, headerH, titleH, videoH, videoTop, videoW, titleOverlayPath ?? undefined, (pct) => {
            ...
            }, isShort, video_speed, gpuTier, metadata.backgroundType, metadata.backgroundColor, metadata.backgroundImage, audioCodec, audioBitrate, headerOl?.src, titleOl, fps_target);
