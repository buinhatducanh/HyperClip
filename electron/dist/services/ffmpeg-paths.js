...
// Helper: run FFmpeg/FFprobe command with proper stderr capture on Windows.
// On Windows, execSync's default behavior doesn't redirect stderr correctly for FFmpeg
// (which uses stderr for all output). Wrap through cmd.exe /c to ensure 2>&1 works.
function ffmpegExec(args, timeout = 8000) {
    const cmdStr = args.map(a => `"${a}"`).join(' ');
    // cmd /c ensures 2>&1 in the command string is interpreted by the shell
    return execSync(`cmd /c ${cmdStr} 2>&1`, { encoding: 'utf-8', timeout, maxBuffer: 10 * 1024 * 1024 }).toString();
}
...
export function getFfmpegVersion(ffmpegPath) {
    ...
    // CUDA filters (scale_cuda/overlay_cuda) need a hardware decoder: NVDEC (modern) or CUVID (legacy).
    // NVDEC: RTX 30/40/50 series desktop GPUs (modern).
    // CUVID: RTX 4050 laptop, older GPU generations (legacy fallback).
    // Both work with scale_cuda/overlay_cuda — the filters consume decoded frames from either path.
    const hasNvDec = result.hasNvdec;
    const hasCuvid = result.hasCuvid;
    try {
        ...
        // Enable CUDA filters if we have them listed AND a compatible hardware decoder (NVDEC or CUVID)
        result.hasCudaFilters = hasCudaFiltersListed && (hasNvDec || hasCuvid);
        if (result.hasCudaFilters) {
            const decoder = hasNvDec ? 'NVDEC' : 'CUVID';
            devLog(`[FFmpeg] CUDA-accelerated filters detected (scale_cuda, overlay_cuda) + ${decoder} decode — GPU filter pipeline enabled`);
        }
        else if (hasCudaFiltersListed && !hasNvDec && !hasCuvid) {
            devLog(`[FFmpeg] CUDA filters listed but no hardware decoder found — using CPU filter pipeline`);
        }
    }
    catch { }
    _cachedVersion = result;
