...
import os from 'os';
import { getAppStoreDir, getRamDiskPath, getChannelListPath, getSeenVideosPath, getChannelsDir, getProjectsDir, getDownloadsDir, } from './paths.js';
...
function getVideoStorageDir() {
    try {
        if (fs.existsSync(getRamDiskPath()))
            return getRamDiskPath();
    }
    catch { }
...
function loadChannels() {
    ...
    try {
        _channelCache = JSON.parse(fs.readFileSync(CHANNELS_FILE, 'utf-8'));
        _channelCacheAt = now;
        return _channelCache;
    }
    catch {
...
export function _invalidateChannelCache() {
    _channelCache = null;
    _channelCacheAt = 0;
}
...
export function getWorkspacesByStatus(status) {
    return getWorkspaces().filter(ws => ws.status === status);
}
// Get workspaces by channel
export function getWorkspacesByChannel(channelId) {
...
export function clearDoneWorkspaces() {
    const store = loadStore();
...
function loadRendered() {
    ensureDir();
    if (!fs.existsSync(RENDERED_FILE))
        return [];
    try {
        return JSON.parse(fs.readFileSync(RENDERED_FILE, 'utf-8'));
    }
    catch {
        return [];
    }
...
export function getRenderedVideos() {
    return loadRendered();
}
export function addRenderedVideo(video) {
    const all = loadRendered();
    all.unshift(video); // newest first
    const pruned = pruneRenderedVideos(all);
    saveRendered(pruned);
}
export function removeRenderedVideo(id) {
    const all = loadRendered();
    const filtered = all.filter(v => v.id !== id);
    saveRendered(filtered);
    return filtered.length !== all.length;
}
export function getRenderedVideosByChannel(channelId) {
    return loadRendered().filter(v => v.channelId === channelId);
}
export function loadSeenVideos() {
    try {
...
export function saveSeenVideos(store) {
    try {
        if (!fs.existsSync(STORE_DIR)) {
            fs.mkdirSync(STORE_DIR, { recursive: true });
        }
...
export function markVideoSeen(channelId, videoId) {
...
export function isVideoSeen(channelId, videoId) {
    const store = loadSeenVideos();
