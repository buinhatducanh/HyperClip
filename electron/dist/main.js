...
import { getWorkspaces, getWorkspace, addWorkspace, updateWorkspace, deleteWorkspace, getChannels, getChannel, addChannel, updateChannel, removeChannel, markVideoSeen, loadSeenVideos, saveSeenVideos, getRenderedVideos, addRenderedVideo, removeRenderedVideo, } from './services/store.js';
import { downloadVideo, getVideoInfo, getChannelInfo } from './services/youtube.js';
import { renderVideo, renderChunked, generateBlurBackground, extractVideoThumbnail, cancelChunked, cancelAllChunked, probeVideoAspect, trimVideo } from './services/ffmpeg.js';
...
import { getVideoStoragePath, getOutputPath, generateWorkspacePaths, cleanupWorkspace, ensureStorageDirs, loadSettings, saveSettings, archiveRenderedFile, openArchiveFolder, showInFolder, getFreeDiskSpace, getRamDiskInfo } from './services/ramdisk.js';
...
import { getOpLogs, clearOpLogs, opLog } from './services/operation_log.js';
                ...
                try {
                    ...
                    if (archiveResult.success && archiveResult.archivedPath) {
                        ...
                        if (workspace.preScaledPath) {
                            try {
                                fs.unlinkSync(workspace.preScaledPath);
                            }
                            catch { }
                        }
...
async function registerIPCHandlers() {
    ...
    ipcMain.handle(IPC_CHANNELS.STORAGE_CLEAR_DOWNLOADS, async () => {
        try {
            ...
            sendNotification('error', `Clear failed: ${err.message}`, undefined);
    ...
    if (cleanupDays > 0) {
        try {
            ...
            if (_activeWorkspaceId)
                activeIds.add(_activeWorkspaceId);
            let cleaned = 0;
            ...
            if (cleaned > 0)
                devLog(`[AutoCleanup] Done — removed ${cleaned} old video files`);
        }
        catch { }
    }
    // ─── Settings ─────────────────────────────────────────────────────────────────
    ...
    ipcMain.handle(IPC_CHANNELS.WORKSPACE_SPLIT, async (_, id, partMinutes = 10) => {
        try {
            const ws = getWorkspace(id);
            if (!ws)
                return { success: false, error: 'Workspace not found' };
                    ...
                    for (const nw of newWorkspaces) {
                        try {
                            if (nw.downloadedPath)
                                fs.unlinkSync(nw.downloadedPath);
                        }
                        catch { }
                    }
        ...
        try {
            ...
            if (m) {
                if (path.startsWith('/channel/'))
                    ...
                    handle = '@' + m[2]; // bare path → treat as @handle
            }
        }
        catch { }
        // ── Duplicate check ───────────────────────────────────────────────────────────
    ...
    ipcMain.handle(IPC_CHANNELS.RENDER_START, async (_, workspaceId, metadata) => {
        return new Promise((resolve) => {
    ...
    ipcMain.handle(IPC_CHANNELS.RENDER_CANCEL, async (_, workspaceId) => {
    ...
    ipcMain.handle(IPC_CHANNELS.RENDER_CHUNKED, async (_, workspaceId, metadata, config) => {
        const workspace = getWorkspace(workspaceId);
        if (!workspace)
        ...
        const result = await renderChunked(resolvedMetadata, outputDir, effectiveConfig, (progress) => {
    ...
    ipcMain.handle(IPC_CHANNELS.RENDERED_ARCHIVE, async (_, workspaceId, customArchiveDir) => {
        const ws = getWorkspace(workspaceId);
        if (!ws)
            return { success: false, error: 'Workspace not found' };
        if (!ws.outputPath)
        ...
        const result = await archiveRenderedFile(ws.outputPath, ws.channelName, ws.videoTitle, quality, codec, ws.fileSize, ws.duration);
    ...
    ipcMain.handle(IPC_CHANNELS.RENDERED_REMOVE, (_, id) => {
        return { success: removeRenderedVideo(id) };
    });
    ipcMain.handle(IPC_CHANNELS.RENDERED_OPEN_FOLDER, (_, id) => {
        if (id) {
    ...
    ipcMain.handle(IPC_CHANNELS.RENDERED_SET_ARCHIVE_PATH, (_, newPath) => {
        const settings = loadSettings();
    ...
    ipcMain.handle(IPC_CHANNELS.CHANNEL_BULK_ADD, async (_, urls) => {
        ...
        for (const url of urls) {
            ...
            if (!trimmed)
                continue;
            try {
                let name, handle, channelId, avatarUrl;
                try {
    ...
    ipcMain.handle(IPC_CHANNELS.AUTH_OAUTH_SET_CREDS, async (_, clientId, clientSecret) => {
    ...
    ipcMain.handle(IPC_CHANNELS.AUTH_OAUTH_START_PER_PROJECT, async (_, clientId, clientSecret, projectId) => {
    ...
    ipcMain.handle(IPC_CHANNELS.PROJECT_LIST, () => {
        ...
        const projects = tokenStatuses.map(ts => {
            ...
            return {
                ...
                apiKeyStatus: primaryKey?.status || 'unauthorized',
            };
        });
     ...
     */
    ipcMain.handle(IPC_CHANNELS.PROJECT_REAUTHORIZE, async (_, projectId) => {
        const fs = await import('fs');
        const path = await import('path');
...
app.whenReady().then(async () => {
    ...
    if (diag.ffmpeg.ok && (!diag.ffmpeg.hasNvdec || !diag.ffmpeg.hasCudaFilters)) {
        console.warn('[HyperClip] RENDER CHẬM: FFmpeg không có NVDEC/CUDA filters — dùng CPU decode + CPU filter pipeline.');
        sendNotification('info', `Render chậm: FFmpeg không có NVDEC/CUDA. CÀI CAPCUT hoặc dùng FFmpeg build có NVIDIA CUDA để render nhanh gấp 10-20x.`);
    }
