...
function syncPollerState(): void {
  const settings = loadSettings()
  if (!_pollerCallbacks) return
...
function getMaxConcurrentDownloads(): number {
  ...
  if (typeof n === 'number' && n >= 1) return Math.min(n, 10)
...
async function autoDownloadFromWebSub(
  ...
  try {
    ...
    devLog(`[Auto] settings: autoRender=${settings.autoRender} autoRenderEnabled=${autoRenderEnabled} autoSplitMinutes=${settings.autoSplitMinutes} autoSplitParts=${settings.autoSplitParts} trimLimit=${autoTrimLimit}`)
    devLog(`[Auto] auto-split: autoSplitMinutes(${settings.autoSplitMinutes ?? 0}) > 0 && trimLimitMin(${typeof autoTrimLimit === 'number' ? autoTrimLimit : 0}) > 0 = ${(settings.autoSplitMinutes ?? 0) > 0 && (typeof autoTrimLimit === 'number' ? autoTrimLimit : 0) > 0}`)
    ...
    if (autoSplitMinutes > 0 && trimLimitMin > 0) {
      ...
      devLog(`[AutoSplit] Downloading "${title}" (${trimSec}s) split into ${numParts} × ${autoSplitMinutes}m sections in parallel`)
      broadcast(IPC_CHANNELS.ACTIVITY_EVENT, {
      ...
      const dlResult = await downloadSectionsAsSeparate({
        ...
        onProgress: (p) => {
          ...
          if (pctDelta >= 2) {
            ...
            broadcast(IPC_CHANNELS.RENDER_PROGRESS_EVENT, { workspaceId: ws.id, percent: p.percent, speed: p.speed, eta: p.eta })
          }
        },
      })

      if (!dlResult.success || dlResult.files.length === 0) {
        devLog(`[AutoSplit] Download failed: ${dlResult.error}`)
        updateWorkspace(ws.id, { status: 'error' })
        broadcast(IPC_CHANNELS.WORKSPACE_UPDATE_EVENT, getWorkspace(ws.id))
        return
      }
      ...
      devLog(`[AutoSplit] Downloaded ${dlResult.files.length} sections in ${downloadElapsed}s`)
      ...
      const renderPromises = dlResult.files.map(async (file, idx) => {
        ...
        if (idx > 0) {
          ...
          devLog(`[AutoSplit] Created workspace ${newWs.id} for section ${idx + 1}`)
        }

        const aspect = await probeVideoAspect(file.path)
        const isLandscape = !aspect?.isShort

        // Parallel: thumbnail + blur
        const thumbPath = path.join(storagePath, `thumb_${targetWsId}.jpg`)
        const { blurPath } = generateWorkspacePaths(targetWsId)
        const [thumbResult, blurResult] = await Promise.all([
          extractVideoThumbnail(file.path, thumbPath),
          isLandscape
            ? Promise.resolve({ success: true })
            : generateBlurBackground(file.path, blurPath, 1080, 1920, file.duration || undefined),
        ])
        ...
        broadcast(IPC_CHANNELS.WORKSPACE_UPDATE_EVENT, getWorkspace(targetWsId))
        broadcast(IPC_CHANNELS.AUTO_DOWNLOAD_EVENT, { videoId, title: `${ws.videoTitle} [${idx + 1}/${dlResult.files.length}]`, channelName: finalChannelName, detectedAt: new Date().toISOString() })

        // Trigger auto-render for this section
        if (autoRenderEnabled) {
          updateWorkspace(targetWsId, { autoRenderAttempted: true })
          const targetWsData: WorkspaceData = idx === 0 ? ws : getWorkspace(targetWsId)!
          const autoMeta = buildAutoRenderMetadata(
            targetWsData,
            file.path,
            file.duration,
            blurBgPath,
            ...
            { start: 0, end: file.duration },
            file.duration, // chunkDuration = section duration → single chunk per section
          )
          devLog(`[AutoSplit] Render started: section ${idx + 1} (${file.duration}s)`)
          renderQueue.push({
    ...
    if (!audioInfo.hasAudio) {
      devLog(`[Auto] ⚠ WARNING: Downloaded video has NO audio — render will be silent! codec=${audioInfo.audioCodec || 'none'}`)
      opLog.warn('download', `No audio in downloaded video: ${title} (${videoId})`)
    } else {
      devLog(`[Auto] Audio OK: codec=${audioInfo.audioCodec}`)
    }
    ...
    if ((settings.autoSplitParts ?? 1) > 1 && (settings.autoSplitMinutes ?? 0) === 0 && autoRenderEnabled) {
      devLog(`[AutoSplit] Using renderChunked with chunkDuration = ${finalDuration / (settings.autoSplitParts ?? 1)}s (${settings.autoSplitParts} equal parts)`)
    }
    ...
    if (autoRenderEnabled && !updatedWs?.autoRenderAttempted) {
      ...
      if (!sourceVideo || !fs.existsSync(sourceVideo)) {
        ...
        const splitChunkDuration = (settings.autoSplitMinutes ?? 0) === 0 && autoSplitParts > 1
          ...
          : undefined
        const autoMetadata = buildAutoRenderMetadata(
        ...
        if (autoSplitParts > 1 && (settings.autoSplitMinutes ?? 0) === 0 && splitChunkDuration) {
          ...
          const chunked = await renderChunked(
            ...
            (progress) => {
              updateWorkspace(ws.id, { renderProgress: Math.round(progress.percent) })
              broadcast(IPC_CHANNELS.RENDER_PROGRESS_EVENT, progress)
            },
          )
