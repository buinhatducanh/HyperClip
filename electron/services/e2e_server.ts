...
import { URL } from 'url'
import {
  getWorkspaces, deleteWorkspace, updateWorkspace, getWorkspace,
  getChannels, addChannel, removeChannel, type StoredChannel,
} from './store.js'
...
function fail(res: http.ServerResponse, status: number, msg: string) {
...
async function readBody<T>(req: http.IncomingMessage): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      try { resolve(body ? JSON.parse(body) : {} as T) }
...
const renderChecks = new Map<string, Record<string, unknown>>()
...
async function handle(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  try {
    ...
    if (method === 'GET' && (pathname === '' || pathname === 'status')) {
      ...
      for (const ws of workspaces) byStatus[ws.status] = (byStatus[ws.status] || 0) + 1

      ok(res, {
        sessions: { totalCount: 30, readyCount: 0 }, // real count injected by getStatus below
        projects: pmStatus,
        poller: poller ? { active: (poller as unknown as { _active: boolean })._active } : { active: false },
    ...
    if (method === 'POST' && pathname === 'channel/add') {
      ...
      if (!body?.url) { fail(res, 400, 'Missing url field'); return }

      const { getChannelInfo } = await import('./youtube.js')
      const info = await getChannelInfo(body.url)
      if (!info) { fail(res, 400, 'Could not resolve channel from URL'); return }
    ...
    if (method === 'POST' && pathname === 'channel/remove') {
      ...
      if (!body?.id) { fail(res, 400, 'Missing id field'); return }
    ...
    if (method === 'GET' && pathname.match(/^workspace\/[^/]+$/)) {
      ...
      if (!ws) { fail(res, 404, `Workspace ${id} not found`); return }
      ok(res, ws)
      return
    }
    ...
    if (method === 'POST' && pathname.match(/^render\/[^/]+$/)) {
      ...
      const ws = getWorkspace(id)
      if (!ws) { fail(res, 404, `Workspace ${id} not found`); return }
      if (ws.status !== 'ready') { fail(res, 400, `Workspace status is '${ws.status}', must be 'ready'`); return }
    ...
    if (method === 'GET' && pathname.match(/^output\/[^/]+$/)) {
      ...
      if (!outPath) { ok(res, { exists: false, path: null, size: 0 }); return }
      try {
        const stat = fs.statSync(outPath)
        ok(res, { exists: true, path: outPath, size: stat.size })
      } catch { ok(res, { exists: false, path: outPath, size: 0 }) }
      return
    }
    ...
    if (method === 'POST' && pathname === 'cleanup') {
      ...
      for (const ws of workspaces) {
        if (ws.status !== 'rendering') { deleteWorkspace(ws.id); deleted.push(ws.id) }
      }
...
export function startE2EServer(): http.Server {
...
export function stopE2EServer() {
  if (server) { server.close(); server = null; devLog('[E2E Server] Stopped') }
}
