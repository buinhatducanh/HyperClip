...
class CDPClient {
  ...
  async connect(wsUrl: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this._ws.on('message', (data: Buffer | Buffer[]) => this._handleMessage(
  ...
  async send<T = unknown>(method: string, params?: Record<string, unknown>): Promise<T> {
    if (!this._ws || !this._alive) throw new Error('CDP not connected')
    return new Promise((resolve, reject) => {
...
async function getCDPTarget(port: number): Promise<CDPTabInfo | null> {
  try {
    const json = await httpGet(`http://localhost:${port}/json`)
    const tabs: CDPTabInfo[] = JSON.parse(json)
...
function parseCookies(cdpCookies: Array<{ name: string; value: string }> | undefined): YouTubeCookies | null {
  if (!cdpCookies) return null
  const cookies: YouTubeCookies = {} as YouTubeCookies
  for (const c of cdpCookies) {
    if (c.name === 'SAPISID') cookies.SAPISID = c.value
    else if (c.name === '__Secure-1PSID') cookies.PSID = c.value
    else if (c.name === '__Secure-1PSIDTS') cookies.PSIDTS = c.value
    else if (c.name === '__Secure-1PSIDCC') cookies.PSIDCC = c.value
    else if (c.name === 'SOCS') cookies.socs = c.value
  }
  if (cookies.SAPISID && cookies.PSID) return cookies
  return null
}
 ...
 */
export async function cdpOpenChromeForLogin(
  profileId: string,
  timeoutMs = 5 * 60 * 1000,
): Promise<CDPLoginResult> {
  ...
  try {
    const killPs = spawn('powershell', [
      '-ExecutionPolicy', 'Bypass', '-Command',
      `netstat -ano | findstr ":${port}" | findstr LISTENING`,
    ], { stdio: ['pipe', 'pipe', 'pipe'] })
    let netstatOut = ''
    killPs.stdout.on('data', (d) => { netstatOut += d.toString() })
    await new Promise<void>(resolve => killPs.on('close', resolve))
  ...
  let cdpTab: CDPTabInfo | null = null
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const { cookies: cdpCookies } = await client.send<{ cookies: Array<{ name: string; value: string }> }>('Network.getAllCookies')
