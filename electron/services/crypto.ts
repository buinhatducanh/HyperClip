...
export function deriveKey(machineId: string, salt?: Buffer): { key: Buffer; salt: Buffer } {
...
export interface EncryptedBlob {
  ...
  salt: string  // hex
  tag: string   // hex (auth tag, stored separately for clarity)
}

/** Encrypt plaintext with AES-256-GCM. Returns a serializable object. */
export function encrypt(plaintext: string, machineId: string): EncryptedBlob {
...
export function decrypt(blob: EncryptedBlob, machineId: string): string {
  const key = crypto.pbkdf2Sync(
    machineId,
...
export function blobToYAMLString(blob: EncryptedBlob): string {
  return [
...
export function parseYAMLBlob(raw: string): EncryptedBlob {
...
export function verifyHMAC(data: string, expected: string, key: Buffer): boolean {
...
export function rsaVerify(data: string, signature: string, publicKeyPem: string): boolean {
  try {
