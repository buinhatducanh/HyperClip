...
export class ChromeSessionManager {
   ...
   */
  async addSession(): Promise<{ success: boolean; profileId?: string; error?: string }> {
    ...
    // Ensure no collision
    while (this._sessions.find(s => s.profileId === String(nextId))) nextId++
    ...
    if (session1) {
      try {
        const destDefaultDir = path.join(profileDir, 'Default')
        const destNetworkDir = path.join(destDefaultDir, 'Network')
        const destLocalState = path.join(profileDir, 'Local State')
        ...
        if (session1.cookies) {
          fs.writeFileSync(path.join(profileDir, '_hyperclip_cookies.json'), JSON.stringify(session1.cookies))
        }
        ...
        if (fs.existsSync(srcSqlite)) {
          try {
            fs.writeFileSync(path.join(destNetworkDir, 'Cookies'), fs.readFileSync(srcSqlite))
          } catch (e2) {
            devLog(`[SessionManager] addSession: SQLite clone failed: ${e2}`)
          }
        }
        ...
        if (fs.existsSync(srcLocalState)) {
          try {
            fs.writeFileSync(destLocalState, fs.readFileSync(srcLocalState))
          } catch (e2) {
            devLog(`[SessionManager] addSession: LocalState clone failed: ${e2}`)
          }
        }
        ...
        devLog(`[SessionManager] addSession: cookie clone failed: ${e}`)
      }
    }
    ...
    try {
      ...
      if (cookies) this._persistCookiesToFile(profileId, cookies)
    } catch (e) {
      session.error = String(e)
    }

    this._sessions.push(session)
    devLog(`[SessionManager] addSession: added profileId=${profileId}, isLoggedIn=${session.isLoggedIn}, isConsented=${session.isConsented}`)
