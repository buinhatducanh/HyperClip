...
export interface HealthAlert {
  ...
  category: 'innertube' | 'oauth' | 'download' | 'storage' | 'detection'
  message: string
  action?: string
  timestamp: number
}
...
const sentAlerts = new Map<string, number>() // alertId → lastSent timestamp
...
async function checkInnertubeHealth(): Promise<HealthAlert | null> {
  try {
...
async function checkOAuthQuota(): Promise<HealthAlert | null> {
  try {
...
async function checkDiskSpace(): Promise<HealthAlert | null> {
  try {
...
async function checkDownloadFailures(): Promise<HealthAlert | null> {
...
async function checkNoNewVideos(): Promise<HealthAlert | null> {
  if (lastVideoDetectionMs === 0) return null // Not enough data yet
...
export function sendHealthAlerts(alerts: HealthAlert[], window: BrowserWindow) {
  for (const alert of alerts) {
    const lastSent = sentAlerts.get(alert.id) || 0
    if (Date.now() - lastSent < ALERT_COOLDOWN_MS) continue // Still in cooldown
