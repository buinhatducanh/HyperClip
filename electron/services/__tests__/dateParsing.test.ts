...
function parseRelativeDate(relativeStr: string, now: number): number {
  ...
  if (weekMatch) return now - parseInt(weekMatch[1]) * 7 * 24 * 60 * 60 * 1000
...
describe('parseRelativeDate', () => {
  ...
  it('parses single minute correctly', () => {
  ...
  it('parses hours correctly', () => {
    ...
    expect(result).toBe(NOW - 3 * 60 * 60 * 1000)
  })

  it('parses single hour correctly', () => {
  ...
  it('parses months correctly (approx 30 days)', () => {
    const result = parseRelativeDate('2 months ago', NOW)
    expect(result).toBe(NOW - 2 * 2_592_000_000)
  })

  it('parses years correctly (approx 365 days)', () => {
    const result = parseRelativeDate('1 year ago', NOW)
    expect(result).toBe(NOW - 31_536_000_000)
  })

  it('parses ISO date strings', () => {
    const isoStr = '2024-01-15T12:00:00Z'
    const expected = new Date(isoStr).getTime()
    expect(parseRelativeDate(isoStr, NOW)).toBe(expected)
  })

  it('returns 0 for unparseable strings', () => {
    expect(parseRelativeDate('just now', NOW)).toBe(0)
    expect(parseRelativeDate('yesterday', NOW)).toBe(0)
    expect(parseRelativeDate('', NOW)).toBe(0)
  })
  ...
  it('parses embedded minutes even in non-relative strings', () => {
    const result = parseRelativeDate('streams in 10 minutes', NOW)
    expect(result).toBe(NOW - 10 * 60 * 1000)
  })

  it('handles plural forms', () => {
    expect(parseRelativeDate('1 minute ago', NOW)).toBe(NOW - 60_000)
    expect(parseRelativeDate('2 minutes ago', NOW)).toBe(NOW - 120_000)
    expect(parseRelativeDate('1 hour ago', NOW)).toBe(NOW - 3_600_000)
    expect(parseRelativeDate('2 hours ago', NOW)).toBe(NOW - 7_200_000)
  })
})
