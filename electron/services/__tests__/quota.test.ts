...
type KeyStatus = 'healthy' | 'warning' | 'exhausted' | 'error' | 'unauthorized' | 'no_key'
...
function deriveKeyStatus(
  ...
  if (projectStatus === 'unauthorized') return 'unauthorized'
  if (usedToday >= MAX_UNITS || projectStatus === 'exhausted') return 'exhausted'
  const quotaPercent = Math.round((usedToday / MAX_UNITS) * 100)
  if (quotaPercent >= QUOTA_WARNING_PCT) return 'warning'
