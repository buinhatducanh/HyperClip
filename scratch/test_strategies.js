// Using native fetch
// Let's implement parseRelativeTime and the strategies
...
(async () => {
  ...
  const rss = await strategyRSS(channelId);
  console.log("RSS returned", rss.length, "videos:");
  rss.slice(0, 3).forEach(v => console.log(`  - ${v.videoId}: "${v.title}" (publishedAt: ${v.publishedAt})`));

  console.log("--- STRATEGY PLAYLIST ---");
  const pl = await strategyPlaylistHTML(channelId);
  console.log("Playlist returned", pl.length, "videos:");
  pl.slice(0, 3).forEach(v => console.log(`  - ${v.videoId}: "${v.title}" (publishedAt: ${v.publishedAt})`));
})();
