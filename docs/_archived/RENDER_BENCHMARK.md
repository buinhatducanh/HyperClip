...
- Download: phụ thuộc YouTube CDN cap + network (xem bảng E2E bên dưới)
- Render queue scheduling

### E2E Total — Video 8 phút

```
E2E = Detection + Download + Pre-process + Render + Auto-trigger
```

**Video specs:** 8:00, 1080p VP9, ~400 MB file (TV embedded, H.264 remux)

**Download optimization (2026-05-21):** 4 instances × 32 concurrent fragments.

| Network | CDN speed | File 400MB | E2E RTX 5080 | E2E RTX 4050 |
|---|---|---|---|---|
| YouTube CDN (free, 1 instance) | ~75 Mbps | ~43s | **~95s** | **~133s** |
| YouTube CDN (free, 4×32frags) | ~100 Mbps | ~32s | **~84s** | **~122s** |
| 1 Gbps LAN + free CDN | ~100 Mbps | ~32s | **~84s** | **~122s** |
| 1 Gbps LAN + YouTube Premium | ~200 Mbps | ~16s | **~68s** | **~106s** |
| 1 Gbps LAN + Premium + 4×32frags | ~250 Mbps | ~13s | **~65s** | **~103s** |

> **YouTube CDN per-IP cap là bottleneck thật sự.** Không phải băng thông LAN.
> Thay đổi 2026-05-21: 1 instance→ 4 instances × 16frags → 32frags → tiết kiệm ~25% download time.
> Tăng instances/fragments thêm không cải thiện được vì YouTube CDN cap per-IP ~100-200 Mbps.

**Bottleneck analysis:**
```
RTX 5080 E2E ~84s:  CDN(32s, 38%) + Detect(20s, 24%) + Render(22s, 26%) + Preproc(5s, 6%) + Trigger(5s, 6%)
RTX 4050 E2E ~122s: CDN(32s, 26%) + Detect(20s, 16%) + Render(56s, 46%) + Preproc(9s, 7%) + Trigger(5s, 4%)
```

**→ Trên RTX 5080:** CDN là bottleneck lớn nhất (38%). Tối ưu render chỉ cải thiện 26%.
**→ Trên RTX 4050:** Render (46%) + CDN (26%) là 2 bottleneck chính. Chunked mode giảm render từ 56s → 19s → E2E giảm 37s.
