...
YouTubePoller._pollOnce() [5s]
  ...
  └→ fetchSubscriptionFeed(seenVideoIds, sinceMs)
        ...
        │    │
        │    ├─ Round-robin sessions (isConsented=true only)
        │    ├─ getNextClient() — skip sessions: cooldown OR not consented
        │    │   Exponential backoff on errors: 30s → 60s → 120s → 240s → max 5min
        │    │
        ...
        └─ RSS FALLBACK (Tier 3 — Innertube + OAuth exhaust → RSS for top-10 channels)
             └─ https://www.youtube.com/feeds/videos.xml?channel_id=UCxxx
                 (No auth, no quota, ~2min delay on new uploads)
```
...
```
/(\d+)\s*second/   → seconds ago (2026-05-09 added)
...
### 8.5. LockupView Metadata Extraction (2026-05-08 UPDATE — FIXING)

YouTubei.js trả về **LockupView** items (NOT GridVideo/VideoRenderer). Có **2 cấu trúc** khác nhau tùy kênh:

#### Cấu trúc A — `lockupMetadata` (kênh thông thường)
| Field | Path |
|-------|------|
| videoId | `lockupMetadata.content_id` |
| title | `lockupMetadata.content_title?.text` |
| published | `lockupMetadata.published_time_text?.text` |

#### Cấu trúc B — `metadata.metadata` (Zilk Kay, kênh mới)
Cấu trúc này có `lockupMetadata = {}` (EMPTY).

| Field | Path |
|-------|------|
| videoId | `content_id` (top-level) |
| title | `metadata.title` |
| published | `metadata.metadata.metadata_rows[0].metadata_parts[i].text.text` (scan pattern `\d+ (minute|hour|day)`) |

**Fix đã implement:** `extractLockupVideoField()` thử 4 path + regex scan metadata_parts.

**Bug 2026-05-09:** `parseRelativeDate()` thiếu "seconds" regex → video vừa đăng (< 1 phút) = "36 seconds ago" → unparseable → skipped. Fix: thêm `/(\d+)\s*second/` check trước minutes.

**2026-05-09 Fixes:**
- `getNextSession()` filter on `isConsented=true` — only use consented sessions (prevents 401/403 errors)
- SOCS force CAI at all 4 places: `_init`, `_refreshBatch`, `refreshSession`, `openLoginWindow`
- Innertube exponential backoff: 30s → 60s → 120s → 240s → max 5min on consecutive errors
- Tier 3 RSS fallback: Innertube + OAuth exhaust → RSS for top-10 priority channels (no auth, no quota)
- UI: "Innertube Degraded" badge in Settings when Innertube returns 0 videos for 3+ consecutive polls

Chi tiết: `memory/lockupview_structure.md`
