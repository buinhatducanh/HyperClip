...
function registerChannelHandlers(ipcMain) {
    ...
    ipcMain.handle(channels_js_1.IPC_CHANNELS.CHANNEL_PAUSE, async (_, id) => {
        const { pauseChannel } = await Promise.resolve().then(() => __importStar(require('../../services/store.js')));
        const result = pauseChannel(id);
        (0, subscription_feed_js_1.refreshChannelCache)();
        return result;
    });
    ipcMain.handle(channels_js_1.IPC_CHANNELS.CHANNEL_RESUME, async (_, id) => {
        const { resumeChannel } = await Promise.resolve().then(() => __importStar(require('../../services/store.js')));
