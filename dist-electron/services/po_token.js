...
async function navigateAndExtractPoToken(port, videoId) {
    ...
    `;
        try {
            const captureResult = await client.send('Runtime.evaluate', { expression: captureScript, returnByValue: true });
            const captureData = captureResult?.result?.value;
 ...
 */
async function extractPoTokenFromProfile(profileId) {
