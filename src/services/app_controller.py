...
class AppController(QObject):
    def __init__(self, client, bus, parent=None):
        ...
        # Instantiate models
        self.channel_list_model = ChannelListModel(backend=client)
        self.workspace_model = WorkspaceModel(backend=client)
        self.stats_model = SystemStatsModel()
        self.settings_model = SettingsModel(backend=client)
        self.activity_model = ActivityLogModel()
        self.hw_profile_model = HardwareProfileModel()
        self.poller_model = PollerStatusModel(backend=client)
        self.auth_model = AuthStatusModel(backend=client)
        self.session_model = SessionListModel(backend=client)
        self.project_model = ProjectListModel(backend=client)
        self.key_model = KeyListModel(backend=client)
        self.rendered_model = RenderedVideoListModel()
        self.detection_history_model = DetectionHistoryModel(backend=client)
