...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 13, fontWeight: 700, color: colors.textSecondary, marginBottom: 4 }}>{children}</div>
}
  ...
  return (
    <SettingsCard>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: colors.accent, flex: 1 }}>HARDWARE PROFILE</span>
        {activePreset && (
          <span style={{ fontSize: 12, color: colors.success, fontFamily: 'monospace', fontWeight: 700 }}>
            ● {activePreset.label}
          </span>
        )}
        {!data.active && (
      ...
      {/* Detected hardware */}
      <div style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 8, fontFamily: 'monospace' }}>
      ...
      {/* Stats display */}
      {display && (
        <div style={{ fontSize: 12, color: colors.textSecondary, fontFamily: 'monospace', lineHeight: 1.8 }}>
...
/** Hook: local settings state with dirty tracking. Returns local state + handlers. */
function useLocalSettings(initial: AppSettings, onSave: (p: Partial<AppSettings>) => void) {
  ...
  // Re-sync when parent updates (after save or external change) — only when JSON changes
  ...
  return (
    <SettingsCard>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: colors.success, flex: 1 }}>AUTO RENDER</span>
        <Toggle value={s.autoRender} onChange={v => patch({ autoRender: v })} />
      </div>
      ...
      <div style={{ marginBottom: 4 }}>
        ...
        <BtnGroup
          ...
          onChange={v => patch({ autoRenderResolution: v as string })}
        />
      </div>
      ...
      <div style={{ marginBottom: 4 }}>
        ...
        <BtnGroup
          ...
          onChange={v => patch({ autoRenderFPS: v as 30 | 60 })}
        />
      </div>

      <div style={{ marginBottom: 4 }}>
        <SectionLabel>Tốc độ render</SectionLabel>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          {[1.0, 1.1, 1.2, 1.3, 1.5, 1.7, 2.0].map(speed => (
            <button
              key={speed}
              onClick={() => patch({ autoRenderSpeed: speed })}
              style={{
                height: 26, padding: '0 8px', cursor: 'pointer',
                background: s.autoRenderSpeed === speed ? colors.accent + '20' : colors.surface,
                border: `1px solid ${s.autoRenderSpeed === speed ? colors.accent + '44' : colors.border}`,
                borderRadius: 2, fontSize: 12, fontWeight: 700,
                color: s.autoRenderSpeed === speed ? colors.accent : colors.textSecondary,
                fontFamily: 'monospace', transition: 'all 0.1s',
              }}>
              {speed}×
            </button>
      ...
      <div style={{ marginBottom: 4 }}>
        <SectionLabel>Chia video</SectionLabel>
        <div style={{ display: 'flex', gap: 2 }}>
          <button
            onClick={() => patch({ autoSplitMinutes: 0, autoSplitParts: 1 } as any)}
            style={{
              flex: 1, height: 26, cursor: 'pointer',
              background: (s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts <= 1 ? colors.accent + '20' : colors.surface,
              border: `1px solid ${(s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts <= 1 ? colors.accent + '44' : colors.border}`,
              borderRadius: 2, fontSize: 12, fontWeight: 700,
              color: (s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts <= 1 ? colors.accent : colors.textSecondary,
              fontFamily: 'monospace', transition: 'all 0.1s',
            }}>
            Không chia
          </button>
          <button
            onClick={() => patch({ autoSplitMinutes: 0, autoSplitParts: (s as any).autoSplitParts > 1 ? (s as any).autoSplitParts : 2 } as any)}
            style={{
              flex: 1, height: 26, cursor: 'pointer',
              background: (s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts > 1 ? colors.accent + '20' : colors.surface,
              border: `1px solid ${(s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts > 1 ? colors.accent + '44' : colors.border}`,
          ...
          <button
            onClick={() => patch({ autoSplitMinutes: 3, autoSplitParts: 1 } as any)}
            style={{
              flex: 1, height: 26, cursor: 'pointer',
              background: (s as any).autoSplitMinutes > 0 ? colors.accent + '20' : colors.surface,
              border: `1px solid ${(s as any).autoSplitMinutes > 0 ? colors.accent + '44' : colors.border}`,
              borderRadius: 2, fontSize: 12, fontWeight: 700,
              color: (s as any).autoSplitMinutes > 0 ? colors.accent : colors.textSecondary,
              fontFamily: 'monospace', transition: 'all 0.1s',
            }}>
            Chia theo phút
          </button>
      ...
      {(s as any).autoSplitMinutes === 0 && (s as any).autoSplitParts > 1 && (
        <div style={{ marginBottom: 4 }}>
          <SectionLabel>Số phần đều</SectionLabel>
          <BtnGroup
            options={[{ label: '2', value: 2 }, { label: '3', value: 3 }, { label: '4', value: 4 }, { label: '5', value: 5 }]}
            value={(s as any).autoSplitParts ?? 2}
            onChange={v => patch({ autoSplitParts: v as number } as any)}
          />
        </div>
      )}

      {(s as any).autoSplitMinutes > 0 && (
        <div style={{ marginBottom: 4 }}>
          <SectionLabel>Phút mỗi phần</SectionLabel>
          <input
            type="number"
            min={1}
            placeholder="3"
            value={(s as any).autoSplitMinutes ?? 3}
            onChange={e => patch({ autoSplitMinutes: Math.max(1, Number(e.target.value)) } as any)}
            style={{
              width: '100%', height: 26, background: colors.bg, border: `1px solid ${colors.border}`,
              borderRadius: 2, color: colors.accent, fontSize: 12, fontFamily: 'monospace', padding: '0 8px', outline: 'none',
            }}
          />
        </div>
      )}
      ...
      {/* Split preview */}
      {(() => {
        const trimMin = typeof s.defaultTrimLimit === 'number' ? s.defaultTrimLimit : 0
        const mins = (s as any).autoSplitMinutes as number ?? 0
        const parts = (s as any).autoSplitParts as number ?? 1
        if (trimMin <= 0) return null
        if (mins > 0) {
          const numParts = Math.ceil(trimMin / mins)
          return (
            <div style={{ fontSize: 11, color: colors.success, fontFamily: 'monospace', marginBottom: 4, padding: '3px 6px', background: colors.success + '11', borderRadius: 2, border: `1px solid ${colors.success}22` }}>
              → Tải {trimMin}m → {numParts} video × {mins}m
              {numParts > 1 && ` (${Array.from({ length: numParts - 1 }, () => `${mins}m`).join(' + ') + ` + ${trimMin - mins * (numParts - 1)}m`})`}
            </div>
          )
        }
        if (parts > 1) {
          const eachMin = (trimMin / parts).toFixed(1)
          return (
            <div style={{ fontSize: 11, color: colors.success, fontFamily: 'monospace', marginBottom: 4, padding: '3px 6px', background: colors.success + '11', borderRadius: 2, border: `1px solid ${colors.success}22` }}>
              → Tải {trimMin}m → {parts} video đều nhau ({eachMin}m mỗi phần)
            </div>
          )
        }
        return null
      ...
      <div>
        ...
        <input type="text" value={s.autoRenderTitleTemplate} onChange={e => patch({ autoRenderTitleTemplate: e.target.value } as any)}
...
function DownloadCard({ s: initial, onChange }: { s: AppSettings; onChange: (p: Partial<AppSettings>) => void }) {
  ...
  useEffect(() => {
    const load = () => ipc.getPollerStatus().then(setPollerStatus).catch(() => {})
    load()
    const t = setInterval(load, 5000)
    return () => clearInterval(t)
  }, [])

  const pollerActive = !!pollerStatus?.active

  return (
    <SettingsCard>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: colors.textSecondary, letterSpacing: '0.08em', textTransform: 'uppercase' }}>DOWNLOAD</span>
        <button
          disabled={!s.hardwareProfile}
          onClick={() => patch({ pollingEnabled: !s.pollingEnabled } as Partial<AppSettings>)}
          style={{
            padding: '2px 8px', borderRadius: 2, fontSize: 12, fontWeight: 700,
            background: s.pollingEnabled ? colors.success + '22' : colors.surface,
            border: `1px solid ${s.pollingEnabled ? colors.success + '66' : colors.border}`,
            color: s.pollingEnabled ? colors.success : colors.textSecondary,
            cursor: s.hardwareProfile ? 'pointer' : 'not-allowed',
            opacity: s.hardwareProfile ? 1 : 0.5,
          }}
        >
          POLL {s.pollingEnabled ? 'ON' : 'OFF'}
        </button>
      </div>

      {s.pollingEnabled && (
        <div style={{ fontSize: 12, color: pollerActive ? colors.success : colors.warning, marginBottom: 4 }}>
          {pollerActive ? '● ĐANG QUÉT' : '● TẠM DỪNG'}
        </div>
      )}
      ...
      <div style={{ marginBottom: 4 }}>
        ...
        <BtnGroup
          ...
          onChange={v => patch({ autoDownloadQuality: v as string })}
        />
      </div>

      <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginBottom: 4 }}>
        <span style={{ fontSize: 12, color: colors.textSecondary, whiteSpace: 'nowrap' }}>Trim tối đa</span>
        {s.defaultTrimLimit === 'full' ? (
          <button onClick={() => patch({ defaultTrimLimit: 10 })} style={{
            flex: 1, height: 26, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2,
            fontSize: 12, color: colors.accent, cursor: 'pointer', fontFamily: 'monospace', fontWeight: 700, textAlign: 'left', padding: '0 8px',
          }}>FULL — bấm để đặt phút</button>
        ) : (
          <input
            type="number"
            min={1}
            max={180}
            value={s.defaultTrimLimit}
            onChange={e => patch({ defaultTrimLimit: Math.max(1, Number(e.target.value)) })}
            style={{
              flex: 1, height: 26, background: colors.bg, border: `1px solid ${colors.border}`,
              borderRadius: 2, color: colors.accent, fontSize: 12, fontFamily: 'monospace', padding: '0 8px', outline: 'none',
            }}
          />
        )}
        <button onClick={() => patch({ defaultTrimLimit: 'full' })} style={{
          padding: '3px 8px', background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 11,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 6px', background: colors.surface, borderRadius: 2 }}>
        <span style={{ fontSize: 11, color: colors.accent, fontWeight: 600 }}>PIPELINE</span>
  ...
  return (
    <SettingsCard>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, color: colors.accent + '66', fontFamily: 'monospace' }}>{usedMB}MB / {freeGB}GB free</span>
      </div>

      <div style={{ marginBottom: 3 }}>
        <div style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 1 }}>Video path</div>
        <div style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <div style={{ flex: 1, background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 2, padding: '2px 5px', fontSize: 12, color: colors.textSecondary, fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.videoStoragePath || '(default)'}</div>
          <button onClick={async () => { const r = await ipc.pickFolder(s.videoStoragePath); if (r?.path) { onChange({ videoStoragePath: r.path } as any); ipc.updateSettings({ videoStoragePath: r.path }) } }} style={{ padding: '2px 5px', background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 12, color: colors.textSecondary, cursor: 'pointer', flexShrink: 0 }}>📁</button>
        </div>
      </div>

      <div style={{ marginBottom: 4 }}>
        <div style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 1 }}>Output path</div>
        <div style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <div style={{ flex: 1, background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 2, padding: '2px 5px', fontSize: 12, color: colors.textSecondary, fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.outputPath || '(default)'}</div>
          <button onClick={async () => { const r = await ipc.pickFolder(s.outputPath); if (r?.path) { onChange({ outputPath: r.path } as any); ipc.updateSettings({ outputPath: r.path }) } }} style={{ padding: '2px 5px', background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 12, color: colors.textSecondary, cursor: 'pointer', flexShrink: 0 }}>📁</button>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 2, marginBottom: 4 }}>
        <button onClick={() => storageStats?.downloadPath && ipc.openFolder(storageStats.downloadPath)} style={{ flex: 1, height: 24, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 11, color: colors.textSecondary, cursor: 'pointer' }}>Mở thư mục</button>
        <button onClick={async () => { const r = await ipc.clearDownloads(); if (r.success) showToast(`Freed ${r.freedMB}MB`) }} style={{ flex: 1, height: 24, background: colors.surface, border: `1px solid ${colors.error}22`, borderRadius: 2, fontSize: 11, color: colors.error, cursor: 'pointer' }}>Xóa cache</button>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <span style={{ fontSize: 12, color: colors.textSecondary }}>Tự động xóa sau</span>
        <select value={s.downloadsCleanupDays} onChange={e => onChange({ downloadsCleanupDays: Number(e.target.value) } as any)} style={{ height: 24, flex: 1, background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 2, color: colors.accent, fontSize: 12, fontFamily: 'monospace', cursor: 'pointer' }}>
    ...
    <SettingsCard>
      ...
      <div style={{ fontSize: 12, color: colors.textSecondary, lineHeight: 1.7 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {innertubeDegraded && <span style={{ color: colors.warning, fontSize: 12, background: colors.warning + '20', padding: '1px 5px', borderRadius: 2 }}>DEGRADED</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, paddingTop: 8, borderTop: `1px solid ${colors.border}` }}>
          <div>
            ...
            {!settings.hardwareProfile && (
              <div style={{ fontSize: 11, color: colors.warning, marginTop: 1 }}>Cần chọn hardware preset trước</div>
            )}
          </div>
          <button
        ...
        <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'monospace', fontSize: 12, color: colors.textTertiary }}>
...
const SystemCard = memo(function SystemCard() {
  ...
  return (
    <SettingsCard>
      ...
      <div style={{ fontSize: 12, color: colors.textSecondary, lineHeight: 1.7 }}>
        <div>GPU: <span style={{ color: colors.textSecondary }}>{systemStats.gpuName || 'N/A'}</span></div>
        <div>CPU: <span style={{ color: colors.textSecondary }}>{systemStats.cpuName || 'N/A'}</span></div>
        <div>RAM: <span style={{ color: colors.textSecondary }}>{Math.round(systemStats.ramTotal || 0)} GB</span></div>
        <div>Workers: <span style={{ color: colors.textSecondary }}>{systemStats.maxChunkWorkers || 8} · NVENC</span></div>
      </div>
    </SettingsCard>
  )
})
...
export const SettingsPanel = memo(function SettingsPanel({ settings, onSettingsChange, activityEntries = [], onClearActivity }: Props) {
  return (
    <div style={{ flex: 1, background: colors.bg, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: 10, color: colors.textSecondary, fontWeight: 700, padding: '6px 12px', borderBottom: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: colors.bg, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <a href="/settings?tab=sessions" style={{ padding: '2px 6px', border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 12, color: colors.textSecondary, textDecoration: 'none' }}>Sessions</a>
          <a href="/settings?tab=projects" style={{ padding: '2px 6px', border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 12, color: colors.textSecondary, textDecoration: 'none' }}>Projects</a>
          <a href="/settings?tab=keys" style={{ padding: '2px 6px', border: `1px solid ${colors.border}`, borderRadius: 2, fontSize: 12, color: colors.textSecondary, textDecoration: 'none' }}>Keys</a>
          <a href="/settings?tab=diag" style={{ padding: '2px 6px', border: `1px solid ${colors.error}22`, borderRadius: 2, fontSize: 12, color: colors.error + '66', textDecoration: 'none' }}>Diag</a>
        </div>
      </div>
