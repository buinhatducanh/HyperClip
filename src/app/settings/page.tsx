...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'sessions' | 'projects' | 'keys' | 'system' | 'diag' | 'logs' | 'update'>('sessions')
  ...
  const TABS = [
    ...
    { id: 'diag' as const, label: 'Diagnostics', color: colors.warning },
    { id: 'logs' as const, label: 'Logs', color: colors.warning },
  ...
  return (
    <div style={{ height: '100vh', background: colors.bg, fontFamily: 'Inter, sans-serif', color: colors.border, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <Link href="/" style={{ fontSize: 10, color: colors.textSecondary, textDecoration: 'none', fontWeight: 700, letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 4, transition: 'all 0.15s' }}
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = colors.textSecondary; (e.currentTarget as HTMLElement).style.background = 'transparent' }}
        >← Quay lại</Link>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        {activeTab === 'system' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            {/* Hardware Info */}
            <div id="hardware-profile-section" style={{ padding: '12px 20px', background: colors.bg, borderBottom: `1px solid ${colors.border}` }}>
              <div style={{ fontSize: 8, color: colors.textSecondary, fontWeight: 700, letterSpacing: '0.1em', marginBottom: 8 }}>HARDWARE</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 8 }}>
                {/* CPU */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>CPU</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: colors.textSecondary, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: colors.textSecondary, fontFamily: 'monospace', minWidth: 30, textAlign: 'right' }}>
                ...
                {/* RAM */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>SYSTEM RAM</div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: colors.textSecondary, fontFamily: 'monospace', lineHeight: 1.2 }}>
                    {Math.round((systemStats.ramUsed ?? 0) * 10) / 10}
                    <span style={{ fontSize: 9, color: colors.textSecondary, marginLeft: 4 }}>/ {Math.round((systemStats.ramTotal ?? 0) * 10) / 10} GB</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                    <div style={{ flex: 1, height: 6, background: colors.border, borderRadius: 2, overflow: 'hidden' }}>
                      <div style={{
                ...
                {/* GPU */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>GPU</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <div style={{
                      width: 6, height: 6, borderRadius: 1,
                      background: systemStats.gpuEncoder === 'nvenc' ? colors.success : systemStats.gpuEncoder === 'qsv' ? colors.warning : colors.textSecondary,
                      boxShadow: systemStats.gpuEncoder === 'nvenc' ? `0 0 4px ${colors.success}66` : 'none',
                    }} />
                    <span style={{ fontSize: 11, fontWeight: 700, color: colors.textSecondary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, fontSize: 8, color: colors.borderHover, fontFamily: 'monospace', marginTop: 2 }}>
                    <span style={{ color: systemStats.gpuEncoder === 'nvenc' ? colors.success : colors.textSecondary }}>{systemStats.gpuEncoder?.toUpperCase() || 'CPU'}</span>
                ...
                {systemStats.gpuEncoder === 'nvenc' && (
                  <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                    <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>GPU LOAD</div>
                    <div style={{ display: 'flex', gap: 12 }}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: colors.textSecondary, fontFamily: 'monospace' }}>{systemStats.gpuUsage ?? 0}%</div>
                        <div style={{ fontSize: 7, color: colors.borderHover, marginTop: 2 }}>utilization</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: colors.textSecondary, fontFamily: 'monospace' }}>{systemStats.gpuTemp ?? 0}°C</div>
                        <div style={{ fontSize: 7, color: colors.borderHover, marginTop: 2 }}>temperature</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: colors.textSecondary, fontFamily: 'monospace' }}>
                ...
                {/* RAM Disk */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>RAM DISK</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <div style={{
                      width: 6, height: 6, borderRadius: 1,
                      background: systemStats.ramDiskIsAvailable ? colors.success : colors.textSecondary,
                ...
                {/* Workers */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>WORKERS</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                    <span style={{ fontSize: 20, fontWeight: 800, color: systemStats.activeWorkers > 0 ? colors.accent : colors.textSecondary, fontFamily: 'monospace' }}>
                      {systemStats.activeWorkers ?? 0}
                    </span>
                    <span style={{ fontSize: 8, color: colors.textSecondary }}>/ {systemStats.maxChunkWorkers || 2} max</span>
                  </div>
                ...
                {/* Network */}
                <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 6, padding: '10px 12px' }}>
                  <div style={{ fontSize: 8, color: colors.textSecondary, letterSpacing: '0.05em', fontWeight: 700, marginBottom: 6 }}>NETWORK</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <div style={{
                    ...
                    <span style={{ fontSize: 10, color: colors.textSecondary, fontFamily: 'monospace' }}>{systemStats.networkIp || '127.0.0.1'}</span>
                  </div>
                  <div style={{ fontSize: 8, color: colors.borderHover, marginTop: 2 }}>
            ...
            {/* Storage */}
            <div style={{ fontSize: 9, fontWeight: 800, color: colors.textSecondary, letterSpacing: '0.1em', marginBottom: 0, padding: '12px 20px 8px', background: colors.bg }}>STORAGE</div>
            <div style={{ background: colors.bg, borderBottom: `1px solid ${colors.border}` }}>
              <StorageWidget />
            </div>

            {/* About */}
            <div style={{ fontSize: 9, fontWeight: 800, color: colors.textSecondary, letterSpacing: '0.1em', marginBottom: 0, padding: '12px 20px 8px', background: colors.bg }}>ABOUT</div>
